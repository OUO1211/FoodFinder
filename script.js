...
// 從 Local Storage 讀取或初始化次數 (修正 Local Storage 衝突點)
function initializeCount() {
    if (IS_LIMITED_VERSION) {
        // 限制模式：從 Local Storage 讀取次數
        ...
        // 無限制模式：永遠給予最大次數，不使用 Local Storage
        searchCount = MAX_SEARCH_COUNT; 
    }
}
...
function handleSearchResults(results, status) {
    ...
    // 1. 減少次數 (只在 LIMITED 版本中執行，修正 Local Storage 衝突點)
    if (IS_LIMITED_VERSION) {
        searchCount--;
        localStorage.setItem('foodFinderSearchCount', searchCount); 
        updateCountDisplay(); 
    }
...
function updateRestaurantMarkers(map, restaurant) {
    ...
    const contentString = `
        <div style="padding: 5px; max-width: 250px;">
            ...
            <a href="https://maps.google.com/?q=${encodeURIComponent(restaurant.name)}&query_place_id=${restaurant.place_id}" 
