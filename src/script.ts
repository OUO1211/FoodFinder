...
let restaurantMarkers: Array<google.maps.Marker> = [];
let currentInfoWindow: google.maps.InfoWindow | null = null;


// Wasm 相關 (在 app.ts 中宣告，這裡用 declare)

declare var wasmModuleLoaded: boolean;
declare var WasmSelector: any;    
let lastSearchResults: Array<google.maps.places.PlaceResult> = [];
let wasmInitializedForCurrentResults: boolean = false; // 追蹤是否初始化過候選列表
...
function initializeMapComponent(): void { // [7]
    if (map) return;
    const defaultLocation: google.maps.LatLngLiteral = { lat: 22.9970, lng: 120.2127 }; // [8]
    const mapElement = document.getElementById('map'); // [9]
    if (mapElement) { // [10]
        map = new google.maps.Map(mapElement as HTMLElement, {center: defaultLocation, zoom: 15}); // [11]
        mapElement.style.display = 'block';
    }
    getUserLocation(); // [12]
}

window.initMapPlaceholder = function(): void{
    mapsApiLoaded = true;
...
function getUserLocation(): void {
    const defaultLocation: google.maps.LatLngLiteral = { lat: 22.9970, lng: 120.2127 };
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            // 成功回呼：參數需要強型別
            function(position: GeolocationPosition): void {
                userLocation = {
            ...
            // 失敗回呼：參數需要強型別
            function(error: GeolocationPositionError): void {
...
// ==========================================================
function searchRestaurants(): void {
...
function drawSearchCircle(center: google.maps.LatLngLiteral, 
    radius: number,                    
...
// Phase 3: 次數管理 (Cont.)
// ==========================================================
function initializeCount(): void {
    if (IS_LIMITED_VERSION) {
        const storedCount: string | null = localStorage.getItem('foodFinderSearchCount'); 
...
function updateCountDisplay(): void {
    const countContainer: HTMLElement | null = document.getElementById('countContainer'); 

    if (countContainer) { // 確保容器存在
        if (IS_LIMITED_VERSION) {
            countContainer.style.display = 'block';
            
            const countElement: HTMLElement | null = document.getElementById('remainingCount');
            // 注意：要確保按鈕存在才能轉型
...
function runActualSearchLogic(): void{
    if(!userLocation){
...
function updateRestaurantMarkers(map: google.maps.Map, // 修正：地圖實例
    restaurant: google.maps.places.PlaceResult // 修正：餐廳結果物件
): void{
    // 1. 清除舊標記和 InfoWindow
